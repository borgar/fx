...
**_Fx_ reads the two bare spellings as Excel does, and the two quoted ones it does not.** `Alpha:Gamma!SomeName` and `Alpha:Gamma!Table1[Col]` lex as three tokens — a name, the `:` operator, and a prefixed operand — so they parse to a `BinaryExpression` and `parseA1Ref` returns `undefined` for them, exactly as it does for `Sheet1!A1:Sheet2!B2`. The colon of a bare prefix is a sheet-range separator only when a cell reference follows the `!`.

For the quoted spellings, `'Alpha:Gamma'!SomeName` and `'Alpha:Gamma'!Table1[Col]`, _Fx_ still returns `context: [ 'Alpha:Gamma' ]`, and `splitSheetRange` still divides that into `[ 'Alpha', 'Gamma' ]`. What Excel reads there is a workbook file name with no sheet component at all, and _Fx_ has no representation for one — the sheet slot is where a file name would have to go, and it is the wrong slot. So a caller resolving sheet names in front of a defined name or a structured reference should expect that scope to be a workbook file name rather than a pair of sheets, whatever `splitSheetRange` makes of it.
