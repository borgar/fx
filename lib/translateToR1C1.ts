...
import { splitContextXls } from './parseRef.ts';
...
const unquote = (d: string): string => d.slice(1, -1).replace(/''/g, "'");
function quickParseA1 (ref: string): ReferenceA1Xlsx {
  const split = ref.lastIndexOf('!');
  let data: Partial<ReferenceA1Xlsx> = {};
  if (split > -1) {
    data = splitContextXls(ref.startsWith('\'') ? unquote(ref.slice(0, split)) : ref.slice(0, split));
