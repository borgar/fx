 ...
 *
 * By default this runs a slice small enough for the suite. After changing a lexer or the
 * sheet-name grammar, run a large sweep by scaling the same tests up:
 *
 * ```
 * SWEEP_COUNT=200000 npx vitest run lib/sheetRangeProperties.spec.ts
 * SWEEP_COUNT=200000 SWEEP_SEED=7 npx vitest run lib/sheetRangeProperties.spec.ts
 * ```
 *
 * Two qualifications are written into the properties, both for deliberate behaviour rather than a
 * defect the sweep should keep re-reporting: the number and boolean literals of `LITERAL_TYPES`,
 * and the round-trip properties applying only to input a writer could have produced, which is
 * what `isWholeReference` settles.
 */
...
function makeRandom (seed: number): Prng {
  let a = seed >>> 0;
  return () => {
    a = (a + 0x6D2B79F5) >>> 0;
...
function genSheetName (rnd: Prng): string {
...
type PropertyCase = {
  ...
  ref: string,
  /** Index into `WRAPPINGS`: how the fragment is embedded in a formula. */
  wrapping: number,
  r1c1: boolean,
  xlsx: boolean,
  mergeRefs: boolean,
...
function generateCase (rnd: Prng): PropertyCase {
 ...
 * `LITERAL_TYPES`: where the formula path never saw a prefix to begin with, it cannot be
 ...
 */
function sweep (options: SweepOptions): Failure[] {
...
// vitest runs specs under node, but the project's tsconfig has no node types
declare const process: { env: Record<string, string | undefined> };

// The seeds are fixed so that a failure is reproducible from the report alone.
const COUNT = Number(process.env.SWEEP_COUNT ?? 4000);
const SEEDS = process.env.SWEEP_SEED ? [ Number(process.env.SWEEP_SEED) ] : [ 1, 2, 3 ];
