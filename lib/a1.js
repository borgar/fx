...
export function parseA1Ref (ref, { allowNamed = true, allowPartials = false, debug = false } = {}) {
  const d = parseRef(ref, { allowNamed, allowPartials, r1c1: false });
  if (d && (d.r0 || d.name)) {
    let range = null;
    if (d.r0) {
      range = fromA1(d.r1 ? d.r0 + ':' + d.r1 : d.r0, debug);
    }
    if (d.name || range) {
      d.range = range;
      delete d.r0;
