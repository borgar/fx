...
/**
 * @ignore
 * @param {boolean} trimTL Does the range have "lead" trimming?
 * @param {boolean} trimBR Does the range have "tail" trimming?
 * @returns {string} The appropriate range join operator
 */
export function rangeOperator (trimTL, trimBR) {
  if (trimTL && trimBR) {
    return '.:.';
  }
  else if (trimTL) {
...
export function toA1 (range) {
  let { top, left, bottom, right, trimTL, trimBR } = range;
...
export function fromA1 (rangeString) {
  ...
  const trimTL = !!op1 && op1.at(0) === '.';
  const trimBR = !!op1 && op1.at(-1) === '.';
  ...
  return { top, left, bottom, right, $top, $left, $bottom, $right, trimTL, trimBR };
}
...
export function parseA1Ref (refString, { allowNamed = true, allowTernary = false, xlsx = false } = {}) {
  ...
  // XXX: just return a fresh object here...
  if (d && (d.r0 || d.name)) {
    let range = null;
    if (d.r0) {
      // FIXME
