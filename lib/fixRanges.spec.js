...
test('fixRanges basics', t => {
  ...
  const tokensWithRanges = tokenize(
    ...
    { emitRanges: true, mergeRanges: true, allowTernary: true }
  );
  t.deepEqual(fixRanges(tokensWithRanges, { addBounds: true }), [
    { type: 'fx_prefix', value: '=', range: [ 0, 1 ] },
    { type: 'func', value: 'SUM', range: [ 1, 4 ] },
    { type: 'operator', value: '(', range: [ 4, 5 ] },
    { type: 'range_ternary', value: 'A2:B1048576', range: [ 5, 16 ] },
    { type: 'operator', value: ',', range: [ 16, 17 ] },
    { type: 'range_struct', value: 'table[@Foo]', range: [ 17, 28 ] },
    { type: 'operator', value: ')', range: [ 28, 29 ] }
  ], 'updates token position information');
  t.end();
});
